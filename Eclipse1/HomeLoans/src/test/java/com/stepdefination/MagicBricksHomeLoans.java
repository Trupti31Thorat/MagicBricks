...
/*	HomeLoansPageFactory homeLoansPage;
	...
	public void the_user_clicks_on_the_link_in_the_navigation_bar(String string) {
		...
		homeLoansPage.switchToLatestWindow();
	}
...
@Then("the user selects the city where the property is located from the {string} dropdown")
public void the_user_selects_the_city_where_the_property_is_located_from_the_dropdown(String city) {
   SBIpage.selectPropertyCity(city);
}

@Then("the user selects the appropriate option for {string}")
public void the_user_selects_the_appropriate_option_for(String option) {
    SBIpage.selectPropertyFinalized(option);
}

@Then("the user clicks on the {string} button")
public void the_user_clicks_on_the_button(String token) {
