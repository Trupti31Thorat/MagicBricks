...
public class BuyModuleSteps {
    ...
    public void the_user_launches_the_magicbricks_website() throws InterruptedException {
        ...
        BaseSteps.driver = BaseSteps.setupDriver(browser);
    ...
    public void the_user_clicks_on_option(String option) {
        ...
        } else if (option.equalsIgnoreCase("Investment Hotspot")) {
    ...
    public void the_selected_property_should_be_displayed_on_a_new_page() {
        Assert.assertTrue("Property details page did not open.",movePage.isNewPropertyPageOpened() );
    }

    // ========== Scenario 3 - Investment Hotspot Valid ==========
    
    @When("the user clicks on {string}")
    public void the_user_clicks_on(String option) {
        if (option.equalsIgnoreCase("Investment Hotspot")) {
            hotspotPage = new InvestmentHotspotPage(driver);
            PageFactory.initElements(driver, hotspotPage); // for @FindBy support
            hotspotPage.clickInvestmentHotspot(); // No need to hover, done in Background
        }
    }
    @When("the user scrolls down to the request callback form")
    public void the_user_scrolls_down_to_the_request_callback_form() throws InterruptedException {
        hotspotPage = new InvestmentHotspotPage(driver);
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
        wait.until(ExpectedConditions.visibilityOf(hotspotPage.nameInput));
    ...
    // ========== Scenario 4 - Investment Hotspot Invalid ==========
	...
	public void the_user_fills_the_form_with_invalid_details_from_sheet_and_row(Integer int1, Integer int2) {
		hotspotPage = new InvestmentHotspotPage(driver);
...
//    @When("the user fills the form with invalid details from sheet {int} and row {int}")
//    public void the_user_fills_the_form_with_invalid_details_from_sheet_and_row(Integer sheet, Integer row) {
//        ExcelReader reader = new ExcelReader();
//        Map<String, String> data = reader.getRowData(sheet, row);
//        hotspotPage.fillForm(data.get("Name"), data.get("Email"), data.get("Phone"));
//        hotspotPage.submitForm();
//    }
    ...
    public void the_user_should_be_redirected_to_the_locality_results_page() {
        Assert.assertTrue("User is not on the locality results page. ", tipsPage.isOnResultsPage());
    }
}